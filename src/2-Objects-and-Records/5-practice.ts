...
export type UpdateSettings<Base, Overrides> = Omit<Base, keyof Overrides> & Overrides;
...
type result1 = UpdateSettings<{ username: string; theme: "light" }, { theme: "dark" }>;
...
type test2 = Expect<Equal<result2, { username: string } & { volume: 10; notifications: true }>>;

// Test 3: Merge objects with no overlapping properties.
type result3 = UpdateSettings<{ username: string; language: string }, { timezone: string }>;
// Expected type: { username: string; language: string } & { timezone: string }
type test3 = Expect<Equal<result3, { username: string; language: string } & { timezone: string }>>;
