...
type LetterTuple = ["a", "b", "c"];

type MT = {
  [Letter in LetterTuple[number]]: `The letter is ${Letter}`;
  /*   👆                       👆
...
type T7 = {
  //           👇
  [Key in "age" | "name"]: UserType6[Key] | null;
};
...
declare function groupByType<U extends { type: string }>(objects: U[]): GroupByType<U>;
...
type OmitByValue<T, Omitted> = FromEntries2<Exclude<Entries<T>, [any, Omitted]>>;
