...
export function buildResponse<T, V>(base: T, data: V): T & V {
  return { ...base, ...data };
}

// Example 1: Combining a response's status with its data.
const response1 = buildResponse({ status: 200 }, { data: "OK" });
type test1 = Expect<Equal<typeof response1, { status: number } & { data: string }>>;
...
type test4 = Expect<
  Equal<typeof response4, { status: number; error: string } & { retry: boolean; timeout: number }>
>;
