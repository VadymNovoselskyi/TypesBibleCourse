...
// export type ComposeConfigs<Configs> = Configs extends [
//   infer Head extends {},
//   ...infer Tail extends {}[]
// ]
//   ? Simplify<Merge<Head, ComposeConfigs<Tail extends [{}, ...{}[]] ? Tail : []>>>
//   : {};

export type ComposeConfigs<Configs, Acc = {}> = Configs extends [infer Head, ...infer Tail]
  ? ComposeConfigs<Tail, Normalize<Merge<Acc, Head>>>
  : Acc;

// --- Usage Examples ---

// Example 1: Merging two partial configurations
const serverConfig1 = assembleConfig({ host: "localhost", port: 8080 }, { debug: true });
...
function initializeServerConfig() {
  const envConfig = { port: 3000 as const, debug: false as const };
  const fileConfig = { host: "127.0.0.1" as const, debug: true as const };
