...
type ParseVersion<Version> = Version extends `${infer Major}.${infer Minor}.${infer Patch}`
  ? [Major, Minor, Patch]
  : never;
...
type DivideName<Str> = Str extends `${infer First} ${infer Last}`
  ? [First, ...(DivideName<Last> extends unknown[] ? DivideName<Last> : [DivideName<Last>])]
  : Str;
...
type DivideArray<T extends any[]> = T extends [infer Head, ...infer Tail] ? [Head, Tail] : never;
...
type SplitFirstAndLast<NameStr extends string> = NameStr extends `${infer First} ${infer Remainder}`
