...
type RemoveVowels<Str extends string> = Str extends `${infer Head}${infer Tail}`
  ? `${Head extends Vowel ? "" : Head}${RemoveVowels<Tail>}`
  : Str extends Vowel
  ? ""
  : Str;

type ToSKU<Str extends string> = RemoveVowels<Lowercase<Str>>;
